							...
							u32 bl_level)
{
	...
	//mipi_panel_auto_detect failed in lk,set bl_level to 0,add by lixiaojun.
	if (!mdss_panel_get_boot_cfg()) {
		bl_level = 0;
		pr_info("%s: mipi_panel_auto_detect failed in lk,set bl_level to 0\n", __func__);
	}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	//let 2st source truly lcd power_ic_en_pin(49) High Output when system waking.---add by lixiaojun
	if (gpio_is_valid(ctrl->power_ic_en_gpio))
		gpio_direction_output(ctrl->power_ic_en_gpio, 1);
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	//let 2st source truly lcd power_ic_en_pin(49) Low Output when system sleeping.---add by lixiaojun
	if (gpio_is_valid(ctrl->power_ic_en_gpio))
		gpio_direction_output(ctrl->power_ic_en_gpio, 0);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	tmp = of_get_named_gpio(np,
		"qcom,mdss-dsi-power-ic-en-gpio", 0);
	ctrl_pdata->power_ic_en_gpio = tmp;
	if (gpio_is_valid(ctrl_pdata->power_ic_en_gpio))
		gpio_request(ctrl_pdata->power_ic_en_gpio, "lcd_power_ic_en_pin");

	tmp = of_get_named_gpio(np,
		"qcom,mdss-dsi-id-gpio", 0);
	ctrl_pdata->id_gpio = tmp;
	if (gpio_is_valid(ctrl_pdata->id_gpio)) {
		gpio_request(ctrl_pdata->id_gpio, "lcd_id_pin");
		//let 1st source yushun lcd id_pin(49) Low Output.---add by lixiaojun
		gpio_direction_output(ctrl_pdata->id_gpio, 0);
	}
