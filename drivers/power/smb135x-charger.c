...
#define CHG_VCOMP_175MV			0x7
#define CHG_VCOMP_MASK			SMB135X_MASK(2, 0)
...
#define HART_LIMIT_EN_BIT	BIT(5)
...
static struct smb135x_chg	*module_chip = NULL;
static int BatteryTestStatus_enable = 0;
static int modify_current_for_calling = 0;
...
static int smb135x_get_prop_batt_status(struct smb135x_chg *chip)
{
	...
	if (chip->chg_done_batt_full || (smb135x_get_prop_batt_capacity(chip) > 99))
...
static int
smb135x_get_prop_battery_voltage_now(struct smb135x_chg *chip)
{
	int rc = 0;
	struct qpnp_vadc_result results;
...
void runin_work(struct smb135x_chg *chip);
				       ...
				       union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_VOLTAGE_NOW:
        val->intval = smb135x_get_prop_battery_voltage_now(chip);
        break;
	default:
		return -EINVAL;
	}
...
{
	int msec_elapsed = (end_time->tv_sec - start_time->tv_sec) * 1000 +
				 ...
				 unsigned long event, void *data)
{
	...
	int *blank;
    int rc;
	struct smb135x_chg *smb135x_chg =
		container_of(self, struct smb135x_chg, fb_notif);
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
			smb135x_chg && smb135x_chg->client) {
		blank = evdata->data;
		if (*blank == FB_BLANK_UNBLANK){
			rc = smb135x_set_fastchg_current(module_chip, 700);
            if (rc < 0) {
                dev_err(module_chip->dev, "Couldn't set fastchg current = %d\n",rc);
                return rc;
            }
		}else if (*blank == FB_BLANK_POWERDOWN){
		    if (modify_current_for_calling)
                return 0;
			rc = smb135x_set_fastchg_current(module_chip, 2000);
            if (rc < 0) {
                dev_err(module_chip->dev, "Couldn't set fastchg current = %d\n",rc);
                return rc;
            }
...
static int smb135x_hw_init(struct smb135x_chg *chip)
{
	...
	if (chip->iterm_ma != -EINVAL) {
		if (chip->iterm_disabled) {
            ...
            rc = smb135x_masked_write(chip, CFG_3_REG,
							CHG_VCOMP_MASK, CHG_VCOMP_175MV);
			if (rc) {
				dev_err(chip->dev,
					"Couldn't set vcomp rc = %d\n", rc);
				goto free_regulator;
			}
			rc = smb135x_masked_write(chip, CFG_14_REG,
	...
	if (chip->fastchg_ma != -EINVAL) {
		rc = smb135x_set_fastchg_current(chip, 700/*chip->fastchg_ma*/);
		if (rc < 0) {
...
static ssize_t smb135x_battery_test_status_show(struct device *dev,
...
void runin_work(struct smb135x_chg *chip)
{
	...
	if (smb135x_get_prop_batt_capacity(chip) > 80) {
        pr_debug("smb135x_get_prop_batt_capacity(chip) > 80\n");
        rc = __smb135x_usb_suspend(chip, true);
        if (rc)
			dev_err(chip->dev,
				"Couldn't disenable charge rc=%d\n", rc);
	}else {
	    if (smb135x_get_prop_batt_capacity(chip) < 60) {
	    pr_debug("smb135x_get_prop_batt_capacity(chip) < 60\n");
	    rc = __smb135x_usb_suspend(chip, false);
        if (rc)
			dev_err(chip->dev,
...
static ssize_t smb135x_in_calling_status_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", modify_current_for_calling);
}

static ssize_t smb135x_in_calling_status_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int retval = 1;
	unsigned int input;
	if (sscanf(buf, "%u", &input) != 1) {
        modify_current_for_calling = 0;
		goto exit;
	}
	if (input != 1) {
        modify_current_for_calling = 0;
...
static struct device_attribute attrs[] = {
	__ATTR(BatteryTestStatus, S_IRUGO | S_IWUSR | S_IWGRP,
			smb135x_battery_test_status_show, NULL),
	__ATTR(modify_current_for_calling, 0666,
			smb135x_in_calling_status_show,
			smb135x_in_calling_status_store),
};
				                      ...
				                      const struct i2c_device_id *id)
{
	...
	create_debugfs_entries(chip);
	    ...
	    error = fb_register_client(&chip->fb_notif);
	    if (error)
...
qpnp_chg_usb_in_get(char *val, const struct kernel_param *kp)
{
	...
	int rc = -1;
	struct qpnp_vadc_result results;
    module_chip->vadc_dev = qpnp_get_vadc(module_chip->dev, "smb135x");
	...
	if(module_chip != 0)
	{
		rc = qpnp_vadc_read(module_chip->vadc_dev, P_MUX2_1_3, &results);
		if (rc) {
			pr_err("Unable to read vchg rc=%d\n", rc);
			return 0;
		}
        value = div64_s64(results.physical, 800LL) ;
...
module_param_cb(charge_current, &usb_in_uv_param_ops, NULL, 0644);
