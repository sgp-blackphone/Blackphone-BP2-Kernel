...
#define BATTERY_DOWN_MIN_CHANGE_RUN 35          // the min time allow battery change quantity when run
#define BATTERY_DOWN_MIN_CHANGE_SLEEP 2200      // the min time allow battery change quantity when run 30min
#define BATTERY_DOWN_MAX_CHANGE_RUN_AC_ONLINE 2200
...
#define ONLY_CHECK_CAPACITY_PROP 1
int cw_capacity;
struct cw_battery {
...
static u8 config_info[SIZE_BATINFO] = {
	#include "battery_profile_feimaotui_for_cm810.h"
};
...
static int cw_init(struct cw_battery *cw_bat)             
{
        ...
        pr_debug("%s: reg_val & CONFIG_UPDATE_FLG = %d \n",__func__,reg_val & CONFIG_UPDATE_FLG);
        if (!(reg_val & CONFIG_UPDATE_FLG)) {
                dev_dbg(&cw_bat->client->dev, "update flag for new battery info have not set\n");
...
static int cw_get_capacity(struct cw_battery *cw_bat)          
{
        ...
        if (((cw_bat->charger_mode > 0) && (cw_capacity <= (cw_bat->capacity - 1)) && (cw_capacity > (cw_bat->capacity - 9)))
                        || ((cw_bat->charger_mode == 0) && (cw_capacity > (cw_bat->capacity)) && (cw_capacity < (cw_bat->capacity + 4)))) {             // modify battery level swing
...
static void rk_bat_update_status(struct cw_battery *cw_bat)
{
        ...
        if ((POWER_SUPPLY_TYPE_UNKNOWN == mode)||(POWER_SUPPLY_TYPE_BATTERY == mode))
            cw_bat->charger_mode =  POWER_SUPPLY_TYPE_UNKNOWN;
        else if(POWER_SUPPLY_TYPE_USB== mode)
    	...
    	if (cw_bat->batt_psy) {
        	...
        	pr_debug("cw2015 battery power supply is not registered\n");
...
static int cw_bat_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   if (client->dev.of_node) {
    ...
    if (cw_bat->ts_pinctrl) {
		...
		if (ret < 0)
			pr_debug("cw2015 Cannot get idle pinctrl state\n");
	}
    err_pinctrl_select:
    if (cw_bat->ts_pinctrl){
