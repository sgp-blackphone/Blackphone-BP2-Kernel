...
struct bmm_client_data {
	...
	struct bmm050_mag_s32_data_t value;
	u8 enable:1;
	s8 op_mode:4;
	u8 odr;
	u8 rept_xy;
...
static void bmm_remap_sensor_data(struct bmm050_mag_s32_data_t *val,
...
static int bmm_wakeup(struct i2c_client *client)
{
	...
	while (try_times) {
		err = bmm_i2c_write(client,
				BMM_REG_NAME(POWER_CONTROL), (u8 *)&value, 1);
		mdelay(BMM_I2C_WRITE_DELAY_TIME);
		dummy = 0;
		err = bmm_i2c_read(client, BMM_REG_NAME(POWER_CONTROL), &dummy, 1);
		if (value == dummy)
			break;
...
static void bmm_work_func(struct work_struct *work)
{
	...
	BMM_CALL_API(read_mag_data_XYZ_s32)(&client_data->value);
...
static int bmm_set_odr(struct i2c_client *client, u8 odr)
{
	...
	err = BMM_CALL_API(set_data_rate)(odr);
	mdelay(BMM_I2C_WRITE_DELAY_TIME);
...
static int bmm_get_odr(struct i2c_client *client, u8 *podr)
{
	...
	err = BMM_CALL_API(get_data_rate)(&value);
	if (!err)
		*podr = value;
		...
		struct device_attribute *attr, char *buf)
{
	...
	BMM_CALL_API(get_power_mode)(&power_mode);
	if (power_mode) {
		mutex_lock(&client_data->mutex_op_mode);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (power_mode) {
		...
		err = BMM_CALL_API(get_rep_XY)(&data);
		...
		const char *buf, size_t count)
{
	...
	if (power_mode) {
		mutex_lock(&client_data->mutex_rept_xy);
		err = BMM_CALL_API(set_rep_XY)(data);
		if (!err) {
			mdelay(BMM_I2C_WRITE_DELAY_TIME);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (power_mode) {
		...
		err = BMM_CALL_API(get_rep_Z)(&data);
		...
		const char *buf, size_t count)
{
	...
	if (power_mode) {
		mutex_lock(&client_data->mutex_rept_z);
		err = BMM_CALL_API(set_rep_Z)(data);
		if (!err) {
			mdelay(BMM_I2C_WRITE_DELAY_TIME);
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct bmm_client_data *client_data = input_get_drvdata(input);
	int count;
	struct bmm050_mag_s32_data_t value = {0, 0, 0, 0, 0};

	if(client_data->op_mode == BMM_VAL_NAME(SUSPEND_MODE)) return 0;
	BMM_CALL_API(read_mag_data_XYZ_s32)(&value);
	if (value.data_ready) {
...
static ssize_t bmm_show_value_raw(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct bmm050_mag_data_s16_t value;
	int count;
	struct input_dev *input = to_input_dev(dev);
	struct bmm_client_data *client_data = input_get_drvdata(input);
		...
		const char *buf, size_t count)
{
	...
	if (BMM_SELF_TEST == data) {
		...
		mdelay(3);
		err = BMM_CALL_API(get_selftest_XYZ)(&dummy);
...
static int bmm_restore_hw_cfg(struct i2c_client *client)
{
	...
	mutex_lock(&client_data->mutex_odr);
	BMM_CALL_API(set_data_rate)(client_data->odr);
	mdelay(BMM_I2C_WRITE_DELAY_TIME);
	mutex_unlock(&client_data->mutex_odr);

	mutex_lock(&client_data->mutex_rept_xy);
	err = bmm_i2c_write(client, BMM_REG_NAME(REP_XY),
			&client_data->rept_xy, 1);
	mdelay(BMM_I2C_WRITE_DELAY_TIME);
	err = bmm_i2c_read(client, BMM_REG_NAME(REP_XY), &value, 1);
	PINFO("BMM_NO_REPETITIONS_XY: %02x", value);
	mutex_unlock(&client_data->mutex_rept_xy);

	mutex_lock(&client_data->mutex_rept_z);
	err = bmm_i2c_write(client, BMM_REG_NAME(REP_Z),
			&client_data->rept_z, 1);
	mdelay(BMM_I2C_WRITE_DELAY_TIME);
	err = bmm_i2c_read(client, BMM_REG_NAME(REP_Z), &value, 1);
...
static int sensor_set_power(struct bmm_client_data *data, int enable)
{
    //return 1;// remove poweron func,add for test by pangkun@longcheer
       int ret = 0;
       if (enable) {
        	data->vdd_reg = regulator_get(&data->client->dev, "vdd");
        	if(IS_ERR(data->vdd_reg)){
        		printk("Error of setting voltage\n");
        		return -1;
        	} else {
        	    regulator_set_voltage(data->vdd_reg, 2850000, 2850000);
                  ret = regulator_enable(data->vdd_reg);
        	}

        	data->vio_reg = regulator_get(&data->client->dev, "vio");
        	if(IS_ERR(data->vio_reg)) {
        		printk("Error of setting voltage\n");
        		return -1;
        	} else {
        	    regulator_set_voltage(data->vio_reg, 1800000, 1800000);
                  ret = regulator_enable(data->vio_reg);
        	}
       } else {
		regulator_disable(data->vdd_reg);
		regulator_disable(data->vio_reg);
              regulator_put(data->vdd_reg);
              regulator_put(data->vio_reg);
	}
	return 0;
}
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (NULL == client_data) {
       ...
       //power on
	sensor_set_power(client_data,1);
...
static int bmm_suspend(struct i2c_client *client, pm_message_t mesg)
{
    ...
    sensor_set_power(client_data,0);
	mutex_unlock(&client_data->mutex_power_mode);
