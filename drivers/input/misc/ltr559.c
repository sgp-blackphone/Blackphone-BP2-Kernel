...
static u32 ps_state_last = 2;	// default 2 means none set
...
static int ltr559_gpio_irq(struct ltr559_data *data)
{
    ...
    if (request_irq(data->irq, ltr559_irq_handler, IRQ_TYPE_LEVEL_LOW, LTR559_DRV_NAME, data)) {
...
static DEVICE_ATTR(poll_delay, SYS_AUTHORITY, ltr559_show_poll_delay_als,
...
static ssize_t ltr559_ps_self_caliberate(struct sensors_classdev *sensors_cdev)			
{
	...
	if(noise<30){
		pdata->prox_threshold  = noise+100;//70;
		pdata->prox_hsyteresis_threshold = noise+90;//60;
	}else if(noise < 50){
		pdata->prox_threshold = noise+105;//70;
		pdata->prox_hsyteresis_threshold = noise+95;//;
	}else if(noise < 100){
		pdata->prox_threshold = noise+110;//85;
		pdata->prox_hsyteresis_threshold = noise+100;//75;
	}else if(noise < 200){
		pdata->prox_threshold = noise+130;//85;
		pdata->prox_hsyteresis_threshold = noise+120;//45;
	}else if(noise < 300){
		pdata->prox_threshold = noise+150;//90;
		pdata->prox_hsyteresis_threshold = noise+140;//50;
 	}else if(noise < 700){
		pdata->prox_threshold = noise+200;//85;
		pdata->prox_hsyteresis_threshold = noise+180;//45;
	}else if(noise < 1300){
		pdata->prox_threshold = noise+300;//90;
		pdata->prox_hsyteresis_threshold = noise+250;//50;
	}else{
	...
	for (i = 2; i < sizeof(reg_tbl)/sizeof(reg_tbl[0]); i++) {
		...
		if (reg_tbl[i].defval != reg_tbl[i].curval) {
			if (i < 10) {
                ...
                if (retval)
                    printk("ltr559_device_init: write addr (%x)  value (%x) failed!  \n", reg_tbl[i].addr, reg_tbl[i].curval);
			} else {
				retval = i2c_smbus_write_byte_data(client, reg_tbl[i].addr, reg_tbl[i].curval & 0xff);
                if (retval)
                    printk("ltr559_device_init: write addr (%x)  value (%x) failed!  \n", reg_tbl[i].addr, reg_tbl[i].curval & 0xff);
				retval = i2c_smbus_write_byte_data(client, reg_tbl[i].addr + 1, reg_tbl[i].curval >> 8);
                if (retval)
                    printk("ltr559_device_init: write addr (%x)  value (%x) failed!  \n", reg_tbl[i].addr + 1, reg_tbl[i].curval >> 8);
			}
		}
	}

	retval = i2c_smbus_write_byte_data(client, 0x1b,0x88);
	if (retval) {
	    printk("ltr559_device_init: write addr (%x)  value (%x) failed!  \n", 0x1b, 0x88);
	}
	retval = i2c_smbus_write_byte_data(client, 0x11,0x88);
	if (retval) {
	    printk("ltr559_device_init: write addr (%x)  value (%x) failed!  \n", 0x11, 0x88);
	}
