				...
				struct synaptics_rmi4_platform_data *rmi4_pdata)
{
	...
	rmi4_pdata->power_gpio = of_get_named_gpio_flags(np,
			"synaptics,power_ldo-gpio", 0, &rmi4_pdata->power_flags);
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
	    dev_err(&rmi4_data->i2c_client->dev, "%s: Failed to send command reset\n", __func__);
	    return retval;
	}
					...
					bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(rmi4_data->board->power_gpio)) {
			retval = gpio_request(rmi4_data->board->power_gpio,
				"power-ldo_gpio");
			if (retval) {
				dev_err(&rmi4_data->i2c_client->dev,
					"unable to request gpio [%d]\n",
					rmi4_data->board->power_gpio);
			...
			gpio_set_value(rmi4_data->board->power_gpio, 1);
            msleep(20);
		} else {
			dev_err(&rmi4_data->i2c_client->dev,
				"irq gpio not provided\n");
		...
		if (rmi4_data->board->disable_gpios) {
			...
			//printk("%s: wxun free TP power(2.8&1.8v)\n", __func__);
			if (gpio_is_valid(rmi4_data->board->power_gpio)){
				gpio_set_value(rmi4_data->board->power_gpio, 0);
				gpio_free(rmi4_data->board->power_gpio);
			}
		}
